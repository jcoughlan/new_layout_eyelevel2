     ...
     ^(CMSampleBufferRef imageDataSampleBuffer, NSError *error)
	 {
         if (imageDataSampleBuffer != nil)
         {
			 ...
			 if( kCMVideoCodecType_JPEG == CMFormatDescriptionGetMediaSubType(formatDesc))
			 {
				 ...
				 [self performSelectorOnMainThread:@selector(writeImageToPhotoLibrary:) withObject:[[[UIImage alloc] initWithData:imageData] autorelease] waitUntilDone:YES];
			 }
			 else
			 {
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection
{
    ...
    if( captureOutput == m_videoCaptureOutput ) //we only want to process the sample if it belongs to video capture.
    {
        ...
        [self performSelectorOnMainThread:@selector(sendBufferToGLRenderer:) withObject:[[[NSCVImageBufferRef alloc] initWithCVImageBufferRef:imageBuffer] autorelease] waitUntilDone:false];
    }
...
- (void)dealloc
{
	...
	[m_stillImageCaptureOutput release];
	[m_videoCaptureOutput release];
    [m_CoordinateConverter release];
